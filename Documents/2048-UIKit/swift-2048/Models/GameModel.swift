...
class GameModel : NSObject {
    ...
    var gameboardStack: [SquareGameboard<TileObject>]
    ...
    init(dimension puzzleSize: Int, threshold maxScore: Int, delegate: GameModelProtocol) {
        dimension = puzzleSize
        threshold = maxScore
        self.delegate = delegate
        queue = [MoveCommand]()
        timer = Timer()
        gameboard = SquareGameboard(dimension: puzzleSize, initialValue: .empty)
        gameboardStack = [gameboard]
        super.init()
    }
    ...
    func insertTileAtRandomLocation(withValue value: Int) {
        ...
        // This is the last stage of the move, now's a good time to save the game state
        addToGameboardStack(add: gameboard)
    }
    ...
    func undoButtonHit() {
        if gameboardStack.count > 1 {
            setBoardFromState(with: gameboardStack[1])
            gameboardStack.remove(at: 0)
        } else {
            delegate.shakeTheButton()
        }
    }
